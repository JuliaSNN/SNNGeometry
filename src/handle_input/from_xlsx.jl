...
function get_connection_parameters_from_xlsx(fn)
    ...
    col_type_idx  = findall(!ismissing, conn_sheet[string("A1:",number_to_xlxs_column_name(ncol),1)][:] )
    col_layer_idx = findall(!ismissing, conn_sheet[string("A2:",number_to_xlxs_column_name(ncol),2)][:] )
    col_comp_idx  = findall(!ismissing, conn_sheet[string("A3:",number_to_xlxs_column_name(ncol),3)][:] )
