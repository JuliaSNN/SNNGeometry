...
function generate_connections(df_conn,v; sparseM=true)
    ...
    for conntype in eachrow(df_conn)
        ...
        overview_pre_i = findfirst(    (overview.Type .==conntype.pre_type)     #note the LHS is a column of overview, while the RHS is a single cell of df_conn
                                    .& (overview.Layer.==conntype.pre_layer)    # for each of these
                                    .& (overview.Comp .=="s")
                                    )
        overview_post_i= findfirst(    (overview.Type .==conntype.post_type)
        ...
        prob_f = select_connection_prob_function(conntype.prob_func,conntype.prob_func_p1,conntype.prob_func_p2)
        #the distribution of the strength of said connection:
        str_d  = select_connection_strength_distribution(conntype.str_func,conntype.str_func_p1,conntype.str_func_p2)
        return prob_f

        @printf("\r%3d/%d   %s -> %s", k,n, string(Vector(overview[overview_pre_i,Not("Idx")])), string(Vector(overview[overview_post_i,Not("Idx")])))
